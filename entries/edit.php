<?php
/**
 * Edit Customer Entry Page
 * Customer Management System
 */

$pageTitle = "Edit Customer Entry";
require_once __DIR__ . '/../includes/functions.php';
requireLogin();

$db = Database::getConnection();
$entryId = (int)($_GET['id'] ?? 0);

if ($entryId <= 0) {
    setFlash('error', 'Invalid entry ID.');
    header("Location: index.php");
    exit();
}

// Fetch Entry with Customer info
$stmt = $db->prepare("
    SELECT ce.*, c.name as customer_name, c.mobile as customer_mobile
    FROM customer_entries ce
    JOIN customers c ON ce.customer_id = c.id
    WHERE ce.id = ?
");
$stmt->execute([$entryId]);
$entry = $stmt->fetch();

if (!$entry) {
    setFlash('error', 'Customer entry not found.');
    header("Location: index.php");
    exit();
}

// Dropdown lists
$powers = getActivePowers();
$lensTypes = getActiveLensTypes();
$frameTypes = getActiveFrameTypes();

$error = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrfToken()) {
        $error = "Invalid security token.";
    } else {
        $powerId = !empty($_POST['power_id']) ? (int)$_POST['power_id'] : null;
        $lensTypeId = !empty($_POST['lens_type_id']) ? (int)$_POST['lens_type_id'] : null;
        $frameTypeId = !empty($_POST['frame_type_id']) ? (int)$_POST['frame_type_id'] : null;
        $entryDate = trim($_POST['entry_date'] ?? date('Y-m-d'));
        $price = (float)($_POST['price'] ?? 0);
        $advanced = (float)($_POST['advanced'] ?? 0);
        $notes = trim($_POST['notes'] ?? '');

        if ($price < 0) {
            $error = "Price must be a valid positive number.";
        } elseif ($advanced < 0) {
            $error = "Advanced payment must be a non-negative number.";
        } elseif ($advanced > $price) {
            $error = "Advanced payment cannot be greater than Price!";
        } else {
            $balance = calculateBalance($price, $advanced);
            $status = determineStatus($balance);

            $updateStmt = $db->prepare("
                UPDATE customer_entries 
                SET power_id = ?, lens_type_id = ?, frame_type_id = ?, entry_date = ?, price = ?, advanced = ?, balance = ?, status = ?, notes = ?
                WHERE id = ?
            ");
            $updateStmt->execute([$powerId, $lensTypeId, $frameTypeId, $entryDate, $price, $advanced, $balance, $status, $notes, $entryId]);

            setFlash('success', "Customer Entry #{$entryId} updated successfully.");
            header("Location: ../customers/view.php?id=" . $entry['customer_id']);
            exit();
        }
    }
}

require_once __DIR__ . '/../includes/header.php';
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h3 class="fw-bold mb-1">Edit Customer Entry #<?php echo $entry['id']; ?></h3>
        <p class="text-muted m-0">Customer: <strong><?php echo htmlspecialchars($entry['customer_name']); ?></strong> (<?php echo htmlspecialchars($entry['customer_mobile']); ?>)</p>
    </div>
    <a href="../customers/view.php?id=<?php echo $entry['customer_id']; ?>" class="btn btn-outline-secondary">
        <i class="bi bi-arrow-left me-1"></i> Back to Customer Profile
    </a>
</div>

<?php if (!empty($error)): ?>
    <div class="alert alert-danger py-2 mb-3"><?php echo htmlspecialchars($error); ?></div>
<?php endif; ?>

<div class="card border-0 shadow-sm rounded-3">
    <div class="card-body p-4">
        <form id="customer-entry-form" action="edit.php?id=<?php echo $entry['id']; ?>" method="POST">
            <?php echo csrfField(); ?>

            <div class="row g-3 mb-4">
                <div class="col-12 col-md-4">
                    <label for="power_id" class="form-label">Power Option <span class="text-danger">*</span></label>
                    <select class="form-select" id="power_id" name="power_id" required>
                        <option value="">-- Select Power --</option>
                        <?php foreach ($powers as $p): ?>
                            <option value="<?php echo $p['id']; ?>" <?php echo ($entry['power_id'] == $p['id']) ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($p['name']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-12 col-md-4">
                    <label for="lens_type_id" class="form-label">Lens Type <span class="text-danger">*</span></label>
                    <select class="form-select" id="lens_type_id" name="lens_type_id" required>
                        <option value="">-- Select Lens Type --</option>
                        <?php foreach ($lensTypes as $lt): ?>
                            <option value="<?php echo $lt['id']; ?>" <?php echo ($entry['lens_type_id'] == $lt['id']) ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($lt['name']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-12 col-md-4">
                    <label for="frame_type_id" class="form-label">Frame Type <span class="text-danger">*</span></label>
                    <select class="form-select" id="frame_type_id" name="frame_type_id" required>
                        <option value="">-- Select Frame Type --</option>
                        <?php foreach ($frameTypes as $ft): ?>
                            <option value="<?php echo $ft['id']; ?>" <?php echo ($entry['frame_type_id'] == $ft['id']) ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($ft['name']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>

            <div class="row g-3 mb-4">
                <div class="col-12 col-md-3">
                    <label for="entry_date" class="form-label">Entry Date <span class="text-danger">*</span></label>
                    <input type="date" class="form-control" id="entry_date" name="entry_date" required value="<?php echo htmlspecialchars($entry['entry_date']); ?>">
                </div>
                <div class="col-12 col-md-3">
                    <label for="price" class="form-label">Price (₹) <span class="text-danger">*</span></label>
                    <input type="number" step="0.01" min="0" class="form-control font-monospace fw-bold" id="price" name="price" required value="<?php echo htmlspecialchars($entry['price']); ?>">
                </div>
                <div class="col-12 col-md-3">
                    <label for="advanced" class="form-label">Advanced Payment (₹) <span class="text-danger">*</span></label>
                    <input type="number" step="0.01" min="0" class="form-control font-monospace fw-bold text-success" id="advanced" name="advanced" required value="<?php echo htmlspecialchars($entry['advanced']); ?>">
                </div>
                <div class="col-12 col-md-3">
                    <label for="balance" class="form-label">Calculated Balance (₹)</label>
                    <input type="text" class="form-control font-monospace fw-bold text-danger bg-light" id="balance" name="balance" readonly value="<?php echo htmlspecialchars($entry['balance']); ?>">
                    <small id="balance-warning" class="text-danger fw-bold d-none"></small>
                </div>
            </div>

            <div class="mb-4">
                <label for="notes" class="form-label">Notes / Remarks</label>
                <textarea class="form-control" id="notes" name="notes" rows="2"><?php echo htmlspecialchars($entry['notes'] ?? ''); ?></textarea>
            </div>

            <div class="d-flex justify-content-end gap-2">
                <a href="../customers/view.php?id=<?php echo $entry['customer_id']; ?>" class="btn btn-light border">Cancel</a>
                <button type="submit" class="btn btn-primary px-4 fw-bold shadow-sm">
                    <i class="bi bi-save me-1"></i> Update Entry
                </button>
            </div>
        </form>
    </div>
</div>

<?php require_once __DIR__ . '/../includes/footer.php'; ?>
